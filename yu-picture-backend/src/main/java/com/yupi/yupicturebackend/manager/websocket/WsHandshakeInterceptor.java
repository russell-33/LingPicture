    ...
    public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler, Map<String, Object> attributes) throws Exception {
        ...
        if (request instanceof ServletServerHttpRequest) {
            ...
            if (ObjUtil.isEmpty(spaceId)) {
                log.error("该图片的空间Id不存在，拒绝握手");
                return false;
            }
            Space space = spaceService.getById(spaceId);
            if (ObjUtil.isEmpty(spaceId)) {
                log.error("该图片的空间不存在，拒绝握手");
                return false;
            }
            //如果是团队空间，并且有编辑权限，才能建立连接
            if (space.getSpaceType() != SpaceTypeEnum.TEAM.getValue()) {
                log.error("不能操作私有空间的图片，拒绝握手");
                return false;
            }
            ...
            //设置用户登录信息等属性到websocket中
            attributes.put("user", loginUser);
            attributes.put("userId", loginUser.getId());
            attributes.put("pictureId", Long.valueOf(pictureId));//注意转化为Long类型
        }
        return true;
    }
